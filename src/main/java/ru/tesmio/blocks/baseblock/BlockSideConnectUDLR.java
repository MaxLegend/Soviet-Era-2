...
public class BlockSideConnectUDLR extends BlockSideCustomModel {
    ...
    public BlockSideConnectUDLR(Properties properties, float shadingInside) {
    ...
    private boolean isBlockConnect(IWorld w, BlockPos p) {
        BlockState s = w.getBlockState(p);
        return s.getBlock() instanceof BlockSideConnectUDLR;
    }
    public BlockState updateState(IWorld w, BlockPos p, BlockState s) {
        if (!w.isRemote()) {
        //сделать проверку на фейсинг
                ...
                if (s.get(FACING) == Direction.NORTH || s.get(FACING) == Direction.SOUTH) {
                    ...
                    s = s.with(PANE_LEFT, this.isBlockConnect(w, p.east())).with(FACING, s.get(FACING)).with(PANE_RIGHT, s.get(PANE_RIGHT)).with(PANE_DOWN, s.get(PANE_DOWN));
                }
