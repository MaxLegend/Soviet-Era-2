package ru.tesmio.soviet.recipes;
...
public abstract class OneByOneRecipe implements IOneByOneRecipe {
    ...
    public boolean matches(RecipeWrapper inv, World worldIn) {
        ...
        if (!output.isEmpty() && output.getItem() != this.getRecipeOutput().getItem()) {
            return false;
        } else if (!(inv.getStackInSlot(1).getCount() >= 64)) {
            if (this.getOneByOne()) {
                return this.input.test(inv.getStackInSlot(0));
            } else if (!this.getOneByOne()) {
                return this.input.test(inv.getStackInSlot(0)) && inv.getStackInSlot(0).getCount() >= this.getCountInput();
            }
        }
        return false;
    }
