package ru.tesmio.soviet.blocks.decorative.lamp;
...
import ru.tesmio.soviet.blocks.baseblock.BaseEnumOrientation;
import ru.tesmio.soviet.blocks.decorative.lamp.base.BlockRotLamp;
...
public class FluoLamp3 extends BlockRotLamp {
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerEntity, Hand handIn, BlockRayTraceResult hit) {
        ...
        if (activeItemRight.getItem() == RegItems.PULLER.get()) {
            ...
            state.getBlock();
            spawnAsEntity(worldIn, pos, new ItemStack(RegItems.FLUOLAMP.get(), 1));
    ...
    public BlockState updateState(World w, BlockPos p, BlockState s) {
        if (w.isBlockPowered(p)) {
            ...
            w.playSound(null, p, SovietSounds.FLUO_LAMP.get(), SoundCategory.BLOCKS, 0.40f, 1f);
        }
