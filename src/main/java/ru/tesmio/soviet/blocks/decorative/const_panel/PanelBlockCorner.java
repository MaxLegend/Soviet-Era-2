package ru.tesmio.soviet.blocks.decorative.const_panel;
    ...
    public BlockState getStateForPlacement(BlockItemUseContext context) {
        for (Direction direction : context.getNearestLookingDirections()) {
            if (direction == Direction.UP) {
                ...
                switch (direction) {
                    case NORTH:
                        if (context.getHitVec().y - (double) context.getPos().getY() > 0.5D)
                            return this.getDefaultState().with(FACING, Direction.NORTH).with(HALF, EnumHalf.UPPER);
                        return this.getDefaultState().with(FACING, Direction.NORTH).with(HALF, EnumHalf.LOWER);
                    case SOUTH:
                        if (context.getHitVec().y - (double) context.getPos().getY() > 0.5D)
                            return this.getDefaultState().with(FACING, Direction.SOUTH).with(HALF, EnumHalf.UPPER);
                        return this.getDefaultState().with(FACING, Direction.SOUTH).with(HALF, EnumHalf.LOWER);
                    case EAST:
                        if (context.getHitVec().y - (double) context.getPos().getY() > 0.5D)
                            return this.getDefaultState().with(FACING, Direction.EAST).with(HALF, EnumHalf.UPPER);
                        return this.getDefaultState().with(FACING, Direction.EAST).with(HALF, EnumHalf.LOWER);
                    case WEST:
                        if (context.getHitVec().y - (double) context.getPos().getY() > 0.5D)
                            return this.getDefaultState().with(FACING, Direction.WEST).with(HALF, EnumHalf.UPPER);
