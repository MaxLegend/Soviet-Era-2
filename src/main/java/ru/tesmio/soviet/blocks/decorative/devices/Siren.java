            ...
            if (w.isBlockPowered(p)) {
                switch (s.get(TYPE)) {
                    case BIO:
                        ...
                        w.playSound(null, p, SovietSounds.BIOLOGICAL_ALARM.get(), SoundCategory.BLOCKS, 4.00f, 1f);
                        break;
                    case RAD:
                        w.getPendingBlockTicks().scheduleTick(p, this, 155);
                        w.playSound(null, p, SovietSounds.RADIATION_ALARM.get(), SoundCategory.BLOCKS, 4.00f, 1f);
                        break;
                    case CHEM:
                        w.getPendingBlockTicks().scheduleTick(p, this, 20);
                        w.playSound(null, p, SovietSounds.CHEMICAL_ALARM.get(), SoundCategory.BLOCKS, 4.00f, 1f);
                        break;
                }
            }
        }
