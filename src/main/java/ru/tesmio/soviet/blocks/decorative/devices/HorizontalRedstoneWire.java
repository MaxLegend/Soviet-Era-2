...
import ru.tesmio.soviet.types.PlaceType;
...
public class HorizontalRedstoneWire extends BlockCustomModel {
    public static final EnumProperty<PlaceType> PLACEMENT = EnumProperty.create("place", PlaceType.class);
    ...
    public HorizontalRedstoneWire(Properties properties) {
        super(properties);
        this.setDefaultState(this.stateContainer.getBaseState().with(POWER, 0).with(PLACEMENT, PlaceType.FLOOR)
    ...
    public VoxelShape getShape(BlockState state, IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
        ...
        if (state.get(PLACEMENT) == PlaceType.FLOOR) {
            return compileShape(state, SHP[0], SHP[2]);
        }
        if (state.get(PLACEMENT) == PlaceType.CEILING) {
    ...
    public BlockState getStateForPlacement(BlockItemUseContext context) {
        ...
        for (Direction d : a) {
            if (d == Direction.UP) {
                return this.getDefaultState().with(PLACEMENT, PlaceType.CEILING);
